...
app = Flask(__name__)
app.config.update(SECRET_KEY='x', SQLALCHEMY_DATABASE_URI='sqlite:///m.db', SQLALCHEMY_TRACK_MODIFICATIONS=False, REMEMBER_COOKIE_DURATION=timedelta(30))
db = SQLAlchemy(app)
...
def an(sid):
    ...
    a=[d.get(e-timedelta(i),0) for i in range(30)]; m,std=np.mean(a),np.std(a)
    if std==0: return
    z=(d.get(e,0)-m)/std
...
def r():
    if request.method=='POST':
        ...
        u.set(request.form['p']); db.session.add(u); db.session.commit()
...
def cs():
    if request.method=='POST':
        ...
        db.session.add(s); db.session.commit(); flash('Shop created','success'); return redirect(url_for('ms', shop_id=s.id))
...
def ap(sid):
    ...
    if request.method=='POST':
        ...
        db.session.add(p); db.session.commit(); flash('Product added','success'); return redirect(url_for('ms', shop_id=s.id))
...
def ep(sid,pid):
    ...
    if request.method=='POST':
        ...
        db.session.commit(); flash('Product updated','success'); return redirect(url_for('ms', shop_id=s.id))
    ...
    if request.method=='POST':
        ...
        if p.sid!=s.id: flash('Invalid product','danger')
            ...
            if p.q<q: flash(f'Not enough stock. Only {p.q} {p.u}','danger')
                ...
                p.q-=q; db.session.add(sa); db.session.commit(); an(s.id)
...
def af(sid):
    ...
    preds=m.predict(np.array([[last+i] for i in range(1,8)])); preds=[max(0,round(p)) for p in preds]
...
def mp(): return render_template('marketplace.html', shops=S.query.filter_by(a=True).all())
...
def vs(sid):
    s=S.query.get_or_404(sid); return render_template('view_shop.html', shop=s, products=P.query.filter_by(sid=s.id).all(), currency=cu())
...
@login_required
@ad
def ahp(): return render_template('admin_help.html', messages=Hm.query.order_by(Hm.cr.desc()).all())

@app.route('/admin/reply_message/<int:mid>', methods=['POST'])
@login_required
@ad
def arm(mid):
    h=Hm.query.get_or_404(mid); h.rep=request.form['rep']; h.st=request.form['st']; db.session.commit(); flash('Reply sent','success'); return redirect(url_for('ahp'))

@app.route('/admin/delete_message/<int:mid>')
@login_required
@ad
def adm(mid): db.session.delete(Hm.query.get_or_404(mid)); db.session.commit(); flash('Deleted','success'); return redirect(url_for('ahp'))
...
def esp(sid):
    ...
    ta=sum(x.tot for x in sales); tp=sum(x.pf for x in sales)
    html=render_template('sales_pdf.html', shop=s, sales=sales, total_amount=ta, total_profit=tp,
                         start_date=st.strftime('%d/%m/%Y'), end_date=ed.strftime('%d/%m/%Y'),
                         generation_date=datetime.now().strftime('%d/%m/%Y %H:%M'), currency=cu(), lang=lg())
...
@ad
def admd():
    return render_template('admin_dashboard.html', total_users=U.query.count(), total_shops=S.query.count(),
                           total_sales_all=Sa.query.count(), total_revenue_all=db.session.query(db.func.sum(Sa.tot)).scalar() or 0,
                           users=U.query.order_by(U.cr.desc()).all(), shops=S.query.order_by(S.cr.desc()).all(),
                           help_messages=Hm.query.order_by(Hm.cr.desc()).limit(10).all())
...
@ad
def ats(sid):
    s=S.query.get_or_404(sid); s.a=not s.a; db.session.commit()
